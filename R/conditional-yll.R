...
yll_conditional_poisson_single <- function(data,
  ...
  interval_widths <- c(interval_widths, interval_widths[length(interval_widths)])
...
yll_conditional_rp_single <- function(data,
  ...
  survival <- map_dfr(age_list, function(a_start) {
    ...
    sub |>
      ...
      summarise(
        surv0 = mean(.data[["surv0_cond"]], na.rm = TRUE),
        surv1 = mean(.data[["surv1_cond"]], na.rm = TRUE),
        .groups = "drop"
      ) |>
  ...
  attr(results, "conditional_survival") <- survival
  results
}
