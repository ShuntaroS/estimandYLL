...
開始年齢が上限の90歳と同じなら、積分する区間がないため ERL と YLL は0になります。
...
`B = 0` と指定すると点推定だけを計算するため、図にも信頼区間は表示されません。
...
```{r save-plot-ja, eval=FALSE}
ggplot2::ggsave("yll.png", plot_yll(result), width = 6, height = 4, dpi = 300)
```

## future による並列実行を使います

主関数 `estimate_yll()` は `use_future = TRUE` が既定です。
実際の並列化は、呼び出す側で指定した `future::plan()` に従います。
上の例では `multisession` で2つの R ワーカーを起動し、計算後に元の設定に戻しています。
`use_future = TRUE` だけではワーカーは起動しません。
Poisson 法と Royston–Parmar 法の比較用関数は、現在は逐次実行です。

同じシードと同じ実行方式で繰り返した結果には再現性があります。
`future` を使わない逐次実行と `future` を使う実行とでは、乱数列が異なる場合があります。
